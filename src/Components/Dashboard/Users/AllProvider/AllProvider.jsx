...
// import { useGetAllProviderQuery } from "../../../../Redux/api/authApi";
...
const AllProvider = () => {
  ...
  // const { data: providersData, isLoading } = useGetAllProviderQuery(null);
  ...
  useEffect(() => {
    const fetchStories = async () => {
      try {
        ...
        setProviderData([]); // Set to empty array if fetching fails
      } finally {
        setLoading(false);
      }
    };
...
useEffect(() => {
  if (providerData && providerData?.length) {
    ...
    const filteredProviders = providerData?.filter(
      (provider) => provider.role === "provider" // Replace "admin" with the role you want to filter by
    );

    // Set the provider data if there are any matching providers
    if (filteredProviders?.length > 0) {
      setFilteredProviderData(filteredProviders);
    }
  }
}, [providerData]);

  const filteredData = useMemo(() => {
    if (!searchText) return filteredProviderData;
    return filteredProviderData.filter((item) =>
      item.fullName.toLowerCase().includes(searchText.toLowerCase())
    );
  }, [filteredProviderData, searchText]);
